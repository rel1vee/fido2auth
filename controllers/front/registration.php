 ...
 *
 * AJAX Endpoints:
 *  - action=get_options: Returns PublicKeyCredentialCreationOptions for navigator.credentials.create()
 *  - action=verify: Validates the attestation response and stores the new credential
 */
...
class Fido2AuthRegistrationModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess()
    {
        ...
        try {
            ...
            PrestaShopLogger::addLog('FIDO2 Registration Error: ' . $e->getMessage(), 3, null, 'Fido2Auth');
    ...
    private function verifyRegistration(array $data)
    {
        ...
        try {
            ...
            $clientData = json_decode($challengeManager->base64UrlDecode($challenge), true);
            ...
            $creationOptions = $attestationValidator->createCreationOptions(
                $challengeManager->base64UrlDecode($challengeString),
            ...
            PrestaShopLogger::addLog('FIDO2 Registration Verify Error: ' . $e->getMessage(), 3, null, 'Fido2Auth');
            die(json_encode([
                'success' => false,
    ...
    {
        // Block direct access, this controller is for AJAX registration only
        Tools::redirect('index.php?controller=my-account');
    }
